...
exports.findBlogPagination = (req, res) => {
  try {
    ...
    BlogCategory.findOne({ slug: slug }).exec(function (err, cate) {
      if (cate) {
        Blog.find({ status: true, category: cate._id })
          .countDocuments()
                ...
                if (
                  ...
                  res.send({
                    ...
                    pagination: (response.pagination = {
                      err: `queried page ${response.pagination.currentPage} is >= to maximum page number ${response.pagination.total_pages}`,
                    }),
                  });
                }
              })
...
exports.getBlogSearch = (req, res) => {
  ...
  var condition = title
    ? { title: { $regex: new RegExp(title), $options: "i" }, status: true }
    : { status: true };
  ...
  Blog.find(condition)
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Some error occurred while retrieving Astro.",
      });
    });
};
